...
xdescribe("test the add Attendee endpoint", ()=>{
    ...
    xit("should return a 404 status code if no event with id exist", async()=>{
    
        try {
            await request(app).post("/api/v1/events/test_id/attendees")
        ...
        // expect(async ()=>{ await request(app).post("/api/v1/events/test_id/attendees")
        // .set('Authorization', `Bearer ${process.env.AUTH_TOKEN}`)}).toThrow();
    }), 
...
describe("test the Update Event Endpoint", ()=>{
    ...
    it("should return 404 status code if no event exist with id", async()=>{
        // const response = await request(app).put("/api/v1/events/event_id")
        // .set('Authorization', `Bearer ${process.env.AUTH_TOKEN}`).send(eventPayload);
        
        // expect(response.statusCode).toBe(409);
        expect(async()=>{ await request(app).put("/api/v1/events/event_id")
        .set('Authorization', `Bearer ${process.env.AUTH_TOKEN}`).send(eventPayload) }).toThrowError()
        
    }), 
    
    xit("should return a 403 status code if current user is not the event creator", async()=>{
        // create a dummy user 
        const user = {id:"randomuuid", username:"username"}
        const invalidToken = createJWT(user);
        
        // const response = await request(app).put(`/api/v1/events/${process.env.EVENT_ID}`)
        // .set('Authorization', `Bearer ${invalidToken}`);

        expect(async()=>{ await request(app).put(`/api/v1/events/${process.env.EVENT_ID}`)
        .set('Authorization', `Bearer ${invalidToken}`) }).toThrow();
    }), 

    xit("should return 200 status code and the event details", async()=>{
        const response = await request(app).post(`/api/v1/events/${process.env.EVENT_ID}`)
        .set('Authorization', `Bearer ${process.env.AUTH_TOKEN}`).send(eventPayload)
...
xdescribe("test the get attendees endpoint", ()=>{
    it("should return 404 if no event with id exist", async()=>{

    }), 

    it("should return a 403 status code if current user is not the event creator", async()=>{


    }), 
    it("should return a 200 status code and a list of attendees if the event has any", async()=>{
